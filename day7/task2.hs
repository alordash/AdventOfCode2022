...
main = do
  ...
  print fs
  print sizes
  let usedSpace = maximum sizes
  let lackOfSpace = usedSpace - (availableSpace - requiredSpace)
  let fitForDelete = filter (>= lackOfSpace) sizes
  let answer = minimum fitForDelete
  print answer
